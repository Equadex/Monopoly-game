...
Street_info::Street_info(int pos_x, int pos_y, Street *street, ALLEGRO_BITMAP *image_in, ALLEGRO_BITMAP *image_railroad, ALLEGRO_BITMAP *image_el, ALLEGRO_BITMAP *image_water, ALLEGRO_FONT *normal_text, ALLEGRO_FONT *bigger_text, ALLEGRO_FONT *small_text, bool active) : Question(pos_x, pos_y, image_in, active), streat(street), normal_text(normal_text), bigger_text(bigger_text), small_text(small_text), image_railroad(image_railroad), image_el(image_el), image_water(image_water){
	streat->get_colors(colors); //gets the color for the street
}

void Street_info::draw(){
	if(active){
		char temp[1000];
